 ...
 *
 * NAMA  : Daniel Charisma Christian
 * NIM   : 13525052
 * KELAS : K02
 *
...
int collatz_conjecture(int N) {
	int bit_isodd, neg_one, mask_even, mask_odd, next_odd, next;
	bit_isodd = N & 1;
	neg_one = ~0;
	mask_even = neg_one + bit_isodd;
	mask_odd = ~mask_even;
	next_odd = (N << 1) + N + 1;
	next = (mask_even & (N >> 1)) | (mask_odd & next_odd);
	return next;
}
/**
 * [WAJIB] [INTEGER]
...
int absolute_cinema(int x) {
	int sign_bit, mask_neg;
    sign_bit = (x >> 31) & 1;
    mask_neg = ~(sign_bit + ~0);
    return (mask_neg ^ x) + sign_bit;
}
/**
 * [WAJIB] [FLOAT]
 * 
...
int baNaNa(unsigned f) {
	int mantissa, mant_non_zero, exp, exp_zero;
	mantissa = (f << 9) >> 9;
	mant_non_zero = !!mantissa;
	exp = (~f >> 23) & 0xff;
	exp_zero = !exp;
	return mant_non_zero & exp_zero;
}
/**
 * [WAJIB] [FLOAT]
 * 
...
int six_seven(int x, int y) {
	int delta, delta_is_positive, add9;

	delta = x + (~y);
	delta_is_positive = ~(delta>>31);
	add9 = delta_is_positive & 9;
...
int SIX_SEVEEEEEEEN(int x, int y) {
	int delta, delta_is_positive, add9, signx, signy, sign_equal;
	...
	signx = x>>31;
	signy = y>>31;
	sign_equal = ~(signx ^ signy);

    return 67 + ((~(signx) & (signy) & 9) | (sign_equal & add9));
}
/**
 * [BONUS] [INTEGER]
 * 
...
int unwavering_feelings(int x) {
	int frac_correction, correction, wholediv128, frac, fracx5, fracx5div128;

	wholediv128 = (x >> 7) << 2;

	frac = x & 127;
	fracx5 = (frac << 2) + frac;
	fracx5div128 = fracx5 >> 7;

	frac_correction = (x + (127 & (x>>31))) >> 7;

    return wholediv128 + fracx5div128 + frac_correction;
}
